...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--input_model_path',default='checkpoints/baseline1_bert_base'); p.add_argument('--test_path',default='dataset/test.pkl'); p.add_argument('--output_dir',default='predictions/baseline1_bert_base'); p.add_argument('--batch_size',type=int,default=32); p.add_argument('--device',default='auto'); a=p.parse_args(); d=select_device(a.device); m,t,meta=load_float_checkpoint(a.input_model_path); m.to(d); r=evaluate(m,DataLoader(read_dataset(a.test_path),batch_size=a.batch_size),d,'test'); save_predictions(r,a.output_dir,'qqp_bert_base'); x={k:v for k,v in r.items() if k not in {'predictions','probabilities'}}; x['checkpoint_metadata']=meta; save_json(x,Path(a.output_dir)/'metrics.json'); print(f"QQP test accuracy: {r['accuracy']:.6f}")
