...
class NLIClassifier(nn.Module):
 ...
 def forward(self,input_ids,attention_mask,token_type_ids=None,labels=None):
  ...
  logits=self.classifier(self.hidden(self.backbone(**kw).last_hidden_state[:,0,:])); p=F.softmax(logits,dim=1); return (F.cross_entropy(logits,labels.view(-1)) if labels is not None else None),p
def save_float_checkpoint(model,tokenizer,output_dir,metadata=None):
 out=Path(output_dir); out.mkdir(parents=True,exist_ok=True); model.backbone.save_pretrained(out/'backbone'); tokenizer.save_pretrained(out); torch.save({'hidden':model.hidden.state_dict(),'classifier':model.classifier.state_dict()},out/'classifier_head.pt'); payload={'model_id':model.model_id}; payload.update(metadata or {}); (out/'metadata.json').write_text(json.dumps(payload,indent=2))
def load_float_checkpoint(checkpoint_dir,map_location='cpu'):
 d=Path(checkpoint_dir); meta=json.loads((d/'metadata.json').read_text()) if (d/'metadata.json').exists() else {}; model=NLIClassifier(AutoModel.from_pretrained(d/'backbone'),meta.get('model_id','')); head=torch.load(d/'classifier_head.pt',map_location=map_location); model.hidden.load_state_dict(head['hidden']); model.classifier.load_state_dict(head['classifier']); return model,AutoTokenizer.from_pretrained(d),meta
