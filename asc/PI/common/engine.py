...
def evaluate(model,dataloader,device,desc='eval'):
    model.eval(); loss_sum=acc_sum=0.; steps=correct=total=0; preds=[]; probs=[]; t=time.perf_counter()
    for b in tqdm(dataloader,desc=desc,ncols=100):
        ids=b['ids'].to(device); mask=b['mask'].to(device); tt=b['token_type_ids'].to(device); y=b['target'].to(device).view(-1); loss,p=model(ids,mask,tt,y); pred=p.argmax(1); loss_sum+=loss.item(); acc_sum+=(pred==y).float().mean().item(); steps+=1; correct+=(pred==y).sum().item(); total+=y.numel(); preds.extend(pred.cpu().tolist()); probs.extend(p.cpu().tolist())
    return {'loss_sum':loss_sum,'accuracy':acc_sum/max(steps,1),'sample_accuracy':correct/max(total,1),'elapsed_seconds':time.perf_counter()-t,'examples':total,'predictions':preds,'probabilities':probs}
def save_predictions(result,output_dir,prefix):
    out=Path(output_dir); out.mkdir(parents=True,exist_ok=True); (out/f'pred_{prefix}.txt').write_text('\n'.join(map(str,result['predictions']))+'\n')
    with (out/f'prob_{prefix}.txt').open('w') as fh:
        for r in result['probabilities']: fh.write(' '.join(f'{x:.10g}' for x in r)+'\n')
