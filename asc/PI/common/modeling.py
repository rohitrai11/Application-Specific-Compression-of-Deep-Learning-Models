"""Model definitions/checkpoint helpers for PI baselines."""
...
class PIClassifier(nn.Module):
    def __init__(self,backbone,model_id=""):
        super().__init__(); self.backbone=backbone; self.model_id=model_id; self.num_labels=2; self.classifier=nn.Linear(int(backbone.config.hidden_size),2)
    @classmethod
