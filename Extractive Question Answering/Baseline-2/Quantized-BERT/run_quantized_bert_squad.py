...
MODEL_CLASSES = {
    ...
    'pruned_bert' : (BertConfig, BertForQuestionAnswering, BertTokenizer)
}
...
def main():
    ...
    l = list(enc)
    layers_to_del = [2,4,6,8,10,12] #delete from list l
    indx_layer_to_del = [indx-1 for indx in layers_to_del]
    # Reversing Indices List
    indicesList = sorted(indx_layer_to_del, reverse=True)
    # Traversing in the indices list
    for indx in indicesList:
    	if indx < len(l):
    		l.pop(indx)
    new_enc = nn.ModuleList(l)
