...
class MyBertForQuestionAnswering(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # Linear layer for predicting start and end positions
    ...
    def forward(
        ...
        end_positions=None,
        return_dict=False,  # To match the training/evaluation loop
    ):
        ...
        # Compute the loss if start and end positions are provided
        if start_positions is not None and end_positions is not None:
            # Clamp start and end positions to be within the sequence length
        ...
        # If return_dict is False, return the loss and logits directly
        if not return_dict:
            return (total_loss, start_logits, end_logits)

        # Return the output as a QuestionAnsweringModelOutput
...
MODEL_CLASSES = {
    'bert-base': (BertConfig, MyBertForQuestionAnswering, BertTokenizer),
    'bert-large': (BertConfig, BertForQuestionAnswering, BertTokenizer),
    'tiny-bert' : (BertConfig, BertForQuestionAnswering, BertTokenizer),
    'distilbert' : (DistilBertConfig, DistilBertForQuestionAnswering, DistilBertTokenizer),
    'bert-medium' : (BertConfig, BertForQuestionAnswering, BertTokenizer),
    'bert-mini' : (BertConfig, BertForQuestionAnswering, BertTokenizer),
    'pruned-bert' : (BertConfig, BertForQuestionAnswering, BertTokenizer)
}
    ...
    for e in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if max_steps > 0 and global_step > max_steps:
                ...
                break
        if args.local_rank in [-1, 0]:
            output_dir = os.path.join(args.output_dir, "epoch-{}".format(e))
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            # Take care of distributed/parallel training
            model_to_save = model.module if hasattr(model, "module") else model
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            outputs = model(input_ids=batch[0],attention_mask=batch[1],token_type_ids=batch[2], return_dict=False)
        ...
        for i, feature_index in enumerate(feature_indices):
            ...
            # models only use two.
            if args.model_type.split('_')[0] in ["xlnet", "xlm"] and args.model_type != 'xlnet_simple':
            	...
            	#start_logits, end_logits = output[1], output[2]
            ...
            prediction_file = "Indices_of_predictions.json"
            with open(prediction_file, 'w', encoding='utf-8') as output:
                json.dump(start_logits_list, output, ensure_ascii=False, indent=2)
                json.dump(end_logits_list, output, ensure_ascii=False, indent=2)
...
def main():
    ...
    if args.do_train:
        ...
        if args.do_biased_train or args.do_fewshot_train or args.do_exclude_long_context:
            ...
            print("The size of the resulting training set:",len(train_dataset))
...
'''
Run summary:
wandb:           HasAns_exact 73.49865
wandb:              HasAns_f1 80.31598
wandb:           HasAns_total 5928
wandb:            NoAns_exact 71.92599
wandb:               NoAns_f1 71.92599
wandb:            NoAns_total 5945
wandb:             best_exact 72.71119
wandb:      best_exact_thresh 0.0
wandb:                best_f1 76.11498
wandb:         best_f1_thresh 0.0
wandb:      eval/HasAns_exact 74.0722
wandb:         eval/HasAns_f1 80.97944
wandb:      eval/HasAns_total 5928
wandb:       eval/NoAns_exact 70.34483
wandb:          eval/NoAns_f1 70.34483
wandb:       eval/NoAns_total 5945
wandb:        eval/best_exact 72.20585
wandb: eval/best_exact_thresh 0.0
wandb:           eval/best_f1 75.65452
wandb:    eval/best_f1_thresh 0.0
wandb:             eval/exact 72.20585
wandb:                eval/f1 75.65452
wandb:             eval/total 11873
wandb:                  exact 72.71119
wandb:                     f1 76.11498
wandb:                  total 11873
wandb:             train/loss 0.7137
wandb:               train/lr 0.0
'''
