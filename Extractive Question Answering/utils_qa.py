...
def get_bool_of_biased_dataset(data_name, bias, larger_than, smaller_than, included_in, custom_func, same_as, not_equal, top_k, ids, bool_indices, misclassification, task_type='ex-qa'):
    stat_file = Path(os.environ['analysis']) / f"{task_type}/{bias}_{data_name}.json"
    ...
    misclassification_file = "misclassifications_ids.json"
    misclassification_ids = load_json(misclassification_file)
    
    if misclassification:
        stat_list = [misclassification_ids.get(_id, 0) for _id in ids]
        bool_indices *= torch.tensor(stat_list)
        logger.info(f" Current size of misclassified points of training set: {bool_indices.sum().item()}")
    ...
    if same_as is not None:
        stat_file2 = Path(os.environ['analysis']) / f"{task_type}/{same_as}_{data_name}.json"
    ...
    if not_equal is not None:
        stat_file2 = Path(os.environ['analysis']) / f"{task_type}/{not_equal}_{data_name}.json"
        stat2 = load_json(stat_file2)
        stat_list = [stat.get(_id, -10000) for _id in ids]
        stat_list2 = [stat2.get(_id, 10000) for _id in ids]
        bool_indices *= torch.tensor([ (s != s2) and s is not None for s, s2 in zip(stat_list, stat_list2)])
